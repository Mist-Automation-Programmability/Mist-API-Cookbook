# EVPN (2-Tier Network) with ESI-LAG to Access Layer

This will define a 2-Tier network (Core/Distribution) with ESI-LAG to the access layer.
</br>
...
The API defaults all configuration to ERB type configs, meaning that the lowest hierarchy inside the EVPN fabric will be configured for L3 functions.  In CRB configuration, you will configure the IRB/VRF config at the core level.  Additionally, you need to specify an EVPN option at the site level to specify "routed_at" to tell the API to render things like virtual addresses at the core level vs the distribution level.
<div style="page-break-after: always">



# Scenario 2: EVPN 2-Tier with ERB
...
# Scenario 1: EVPN 2-Tier with CRB
...
When running in CRB, we need to specify the `routed_at` key to tell the API that the L3 gateway functionality is not at the edge.  Currently It is only supported for the level above the edge.  In this example, the `edge` of the fabric would be distribution and the next level up would be `core`.
